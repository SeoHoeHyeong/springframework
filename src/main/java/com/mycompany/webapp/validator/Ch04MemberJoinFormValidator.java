...
public class Ch04MemberJoinFormValidator implements Validator{
	...
	public boolean supports(Class<?> clazz) {//유효성 검사를 할수있는지 없는지 확인
		...
		logger.info("실행");
		boolean check = Ch04Member.class.isAssignableFrom(clazz); //isAssignableFrom에 들어간 객체가 Ch04Member에 대입될수 있나(부모/자식)
		return check;
	}
	...
	public void validate(Object target, Errors errors) {
		...
		if(member.getMid() == null || member.getMid().trim().equals("")){
			...
			if(member.getMid().length() < 4) {
				errors.rejectValue("mid", "errors.mid.minlength", new Object[] {4}, ""); // 키의 값을 넣기 위해서 객체를 생성해준다, 키가 없다면 에러 메세지를 직접 넣어둔다. 
			}
		}
